    ...
    <div id="users">
        <div class="search">
            <el-form :inline="true" label-position="left" :model="condition" label-width="0.56rem">
                <el-form-item label="报废单号">
                    <el-input v-model="condition.sId" placeholder="请输入报废单号"></el-input>
                </el-form-item>
                <el-form-item label="报废建立时间">
                    <el-date-picker
                ...
                <el-form-item label=" ">
                    <el-button @click="page = 1; getUser(page,pageSize)">查询</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="btns">
        ...
        <el-dialog
            ...
            <el-form v-if="isAdd" class="userForm" :inline="true" ref="userForm" :rules="rules" :model="userForm" label-width="70px">
                <div>
                    <el-form-item label="报废人" prop="sUserName">
                        <el-input v-model="userForm.sUserName" placeholder="请输入报废人姓名" :readonly="!isAdd"></el-input>
                    </el-form-item>
                </div>
            ...
            <el-table
                ...
                <el-table-column
                    ...
                    width="230">
                    <template slot-scope="scope">
                        <el-input v-if="isAdd" v-model="scope.row.chargeManName" placeholder="请输入负责人姓名"></el-input>
            ...
            <span v-if="isAdd" slot="footer" class="dialog-footer">
                <el-button type="primary" @click="userFormSubmit">确认报废</el-button>
            </span>
        </el-dialog>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            passwordBF: '',
            rules: {
                sUserName: [{required: true, message: '请输入报废人姓名', trigger: 'blur'}],
            },
            isAdd: true,
            dialogVisible: false,
    ...
    methods: {
        ...
        getUser(current, size) {
            let form = {}
            if(this.condition.sId) form.sId = this.condition.sId
            if(this.condition.time) {
                form.leftDate = this.condition.time[0]
                form.rightDate = this.condition.time[1]
            }
            this.axios({
                url: this.$host + `/${this.alias}/scrapped/list/${current}/${size}`,
                method: 'post',
                data: form
            }).then(res => {
        ...
        editClick(row) {
            ...
            this.isAdd = false;
            this.axios({
                url: this.$host + `/${this.alias}/scrapped/detail/${sId}`,
                method: 'get',
            }).then(res => {
                this.detailList = res.data.data.scrappedDetailList
        ...
        addAjax() {
            ...
            return this.$message.error('请添加报废设备')
            let bool = false,
            list = []
            this.handleList.forEach( el => {
                list.push({pId: el.pId, chargeManName: el.chargeManName})
                if(!el.chargeManName)
                return bool = true
            })
            if(bool)
            ...
            let params = {
                sUserName: this.userForm.sUserName,
                scrappedDetailListStr: JSON.stringify(list)
            };
            this.axios({
                url: this.$host + `/${this.alias}/scrapped/create`,
                method: 'post',
                data: params
            }).then(res => {
                if (res.data.code === 0) {
